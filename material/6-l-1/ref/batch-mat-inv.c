...
// A simple OpenMP pragma on top - valid because each slice of Ms is
// operated on independently.
void batch_matrix_inverse_par(int k, int n, double *Ms) {
...
// Fully distributed and maximally parallelised - note that this does
// not make it particularly *fast*.  All functions had to be inlined
// so we could move around the loops and create the necessary
// expansions.
void batch_matrix_inverse_maximally_par(int k, int n, double *MS) {
  int m = 2*n;
  double *A_expanded = malloc(k * n * m * sizeof(double));
#pragma omp parallel for collapse(3)
  for (int l = 0; l < k; l++) {
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < m; j++) {
        double *A = &A_expanded[l*n*m];
        double *M = &MS[l*n*n];
        if (j < n) {
          A[i*m+j] = M[i*n+j];
        } else {
  ...
  double *irow_expanded = malloc(k*m*sizeof(double));
  double *r_expanded = malloc(k*sizeof(double));
  int *pivot_idx_expanded = malloc(k*sizeof(int));
  double *f_copy_expanded = malloc(k * n * sizeof(double));
  int steps = min(n,m);
  for (int s = 0; s < steps; s++) {
#pragma omp parallel for
    for (int l = 0; l < k; l++) {
      ...
      int pivot_idx = find_pivot(n, m, s, A);
      assert(pivot_idx >= 0);
      r_expanded[l] = (1-A[s*m+s]) / A[pivot_idx*m+s];
      pivot_idx_expanded[l] = pivot_idx;
    }
...
#pragma omp parallel for collapse(2)
    for (int l = 0; l < k; l++) {
      for (int i = 0; i < m; i++) {
        double *A = &A_expanded[l*n*m];
        double *irow = &irow_expanded[l*m];
        int pivot_idx = pivot_idx_expanded[l];
        double r = r_expanded[l];
        irow[i] = r * A[pivot_idx*m+i] + A[s*m+i];
      }
    }
    ...
    for (int l = 0; l < k; l++) {
      for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
          double *A = &A_expanded[l*n*m];
          double *irow = &irow_expanded[l*m];
          double f = f_copy_expanded[l*n+i];
          double x = irow[j];
          double y = A[i*m+j];
          double z;
          if (i == s) {
...
#pragma omp parallel for collapse(3)
  ...
  free(pivot_idx_expanded);
  free(irow_expanded);
  free(r_expanded);
  free(f_copy_expanded);
  free(A_expanded);
}
...
int main() {
  ...
  for (int i = 0; i < num_sizes; i++) {
    ...
    benchmark("Maximally parallel",
              k, n, batch_matrix_inverse_maximally_par, golden);
    printf("\n");
    free(golden);
  }
}
